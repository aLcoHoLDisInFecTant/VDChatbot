...
<head>
  ...
  <title>智能分诊系统</title>
  <style>
    body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,"Noto Sans","PingFang SC","Microsoft YaHei",sans-serif;margin:0;background:#0b1020;color:#e8eaf0}
    .wrap{max-width:900px;margin:0 auto;padding:16px}
    .card{background:#111a33;border:1px solid #1e2a55;border-radius:12px;overflow:hidden}
    .head{padding:12px 14px;border-bottom:1px solid #1e2a55;display:flex;gap:10px;align-items:center}
    .dot{width:10px;height:10px;border-radius:50%;background:#4ade80}
    .title{font-weight:700}
    .sub{opacity:.75;font-size:12px}
    .log{padding:14px;height:60vh;overflow:auto;display:flex;flex-direction:column;gap:10px}
    .msg{padding:10px 12px;border-radius:10px;max-width:85%;white-space:pre-wrap;word-break:break-word;border:1px solid transparent}
    .u{align-self:flex-end;background:#1a2550;border-color:#2a3a7a}
    .a{align-self:flex-start;background:#111827;border-color:#24314f}
    .bar{display:flex;gap:10px;padding:12px;border-top:1px solid #1e2a55}
    textarea{flex:1;resize:none;border-radius:10px;border:1px solid #2a3a7a;background:#0b1020;color:#e8eaf0;padding:10px 12px;min-height:44px;outline:none}
    button{border:1px solid #2a3a7a;background:#1a2550;color:#e8eaf0;border-radius:10px;padding:10px 14px;cursor:pointer;font-weight:600}
    button:disabled{opacity:.6;cursor:not-allowed}
    .row{display:flex;gap:10px;align-items:center;padding:0 12px 12px}
    input{border-radius:10px;border:1px solid #2a3a7a;background:#0b1020;color:#e8eaf0;padding:8px 10px;outline:none}
...
<body>
  <div class="wrap">
    <div class="card">
      <div class="head">
        <div class="dot"></div>
        <div>
          <div class="title">智能分诊系统 Web UI</div>
          <div class="sub">后端：/v1/chat/completions（非流式）</div>
        </div>
      </div>
      <div class="row">
        <div class="hint">用户名</div>
        <input id="username" value="kevin" />
        <div class="hint">会话</div>
        <input id="conversation" style="flex:1" />
        <button id="newConv">新会话</button>
      </div>
      <div id="log" class="log"></div>
      <div class="bar">
        <textarea id="input" placeholder="输入问题，例如：张三九的健康档案信息"></textarea>
  ...
  <script>
    ...
    function uuidv4() {
      ...
      a[6] = (a[6] & 0x0f) | 0x40;
      a[8] = (a[8] & 0x3f) | 0x80;
      const b = Array.from(a).map(x => x.toString(16).padStart(2, "0"));
      return `${b.slice(0,4).join("")}-${b.slice(4,6).join("")}-${b.slice(6,8).join("")}-${b.slice(8,10).join("")}-${b.slice(10,16).join("")}`;
    }

    function ensureConversation() {
      if (!convEl.value) convEl.value = `${usernameEl.value}_${uuidv4()}`;
    }
    ...
    async function send() {
      ...
      const payload = {
        ...
        stream: false,
        userId: usernameEl.value || "user",
        conversationId: convEl.value
      };
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          addMsg("assistant", `HTTP ${resp.status}\\n${t}`);
          return;
        }
        const data = await resp.json();
        const content = data?.choices?.[0]?.message?.content ?? "";
        addMsg("assistant", content || "(空响应)");
      } catch (e) {
    ...
    newConvBtn.addEventListener("click", () => {
      convEl.value = `${usernameEl.value || "user"}_${uuidv4()}`;
    });
